...
function(veg,use=c("columns","rows"),rlimit=5,y=1,x.axis=NULL,y.axis=NULL) {
    ...
    vegr<- as.data.frame(veg[,f.s > 0])
    if(use == "columns") ranktemp <- vegr
    if(use == "rows") ranktemp <- as.data.frame(t(vegr))
    vnames<- names(ranktemp)                # the variable names
    ...
    if(xnull == TRUE) {
        ...
        if(use == "columns") ranktemp <- as.data.frame(t(vegr))
    ...
    for (k in 1:nm) {
        ...
        if(sum(dr) > 0) {
            sst<-ss[proc==TRUE]
            sr2<-colSums(r[proc==TRUE,proc==TRUE]^2)
            drt<-dr[proc==TRUE]
            drt[drt<small]<-small
            ss[proc==TRUE]<-(sst+sr2)/drt
            hr<- which.max(ss)
            hrank[k] <- hr
            hvar[k] <- ss[hr]
