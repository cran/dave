...
# =======================================================================
# plotting vegetation tables based on function image()  vers. 31.8.2012
...
# y is for transformation of gray values
  ...
  nspec <- ncol(veg)
#
# enlarge when tables are no larger that 40 x 40
#
  enlarge=FALSE
  if(nrel<35) {
...
# -------------------
  ...
  for (i in 1:mrow) for (j in 1:mcol) {
...
# ----------------------
  ...
  text(xx,yy,grr[o.px][ir.fr:ir.to],pos=3,srt=90,cex=f2^0.6*0.4)
# species groups (righthand)
# --------------------------
  ytt<- seq(0,(nele.s-1)/wsz,1/wsz)
  xtt<- rep((nele.r+1)/wsz,nele.s)
  text(xtt,ytt,grs[o.py][js.fr:js.to],pos=4,cex=f2^0.6*0.4)
#
# new lines
  rangey<- js.to-js.fr+1
  lines(c(0-hpsize,0-hpsize),c(0-hpsize,(rangey*psize)-hpsize))                          # left
  lines(c(0-hpsize,(nrel*psize)-hpsize),c(0-hpsize,0-hpsize))                            # below
  lines(c(0-hpsize,(nrel*psize)-hpsize),c((rangey*psize)-hpsize,(rangey*psize)-hpsize))  # top
  lines(c((nrel*psize)-hpsize,(nrel*psize)-hpsize),c(0-hpsize,(rangey*psize)-hpsize))    # right
# lines separating the releve groups
  iposy<- 0
  o.set<- setgroupsize(grs[o.py][js.fr:js.to])
  for(k in 1:(o.set$ngroups-0)) {
      iposy<- iposy+o.set$groupcounts[k]*psize
      lines(c(0-hpsize,(nrel*psize)-hpsize),c(iposy-hpsize,iposy-hpsize),col=gray(0.2),lwd=1)
  }
